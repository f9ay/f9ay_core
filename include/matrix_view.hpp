...
enum class View_Type { origin, fixed, runtime };
...
public:
    ...
    explicit ElementIterator_base(Origin_Type *_origin) : origin(_origin) {}
...
class ElementIterator<View_Type::fixed, Origin_Type, Index>
    : public ElementIterator_base<Origin_Type> {
public:
    using ElementIterator_base<Origin_Type>::origin;  // 不知道為什麼有 bug
                                                      // 要加這行
    auto &value() {
        return getValue<Index>(*origin);
    }
    ElementIterator(Origin_Type *_origin)
        : ElementIterator_base<Origin_Type>(_origin) {}
};

template <typename Origin_Type>
class ElementIterator<View_Type::runtime, Origin_Type, 0>
    : public ElementIterator_base<Origin_Type> {
public:
    using ElementIterator_base<Origin_Type>::origin;  // 不知道為什麼有 bug
                                                      // 要加這行
    ...
    ElementIterator(Origin_Type *_origin, const int _index)
        : ElementIterator_base<Origin_Type>(_origin), index(_index) {}
...
// 推導指引
template <typename Origin_Type>
ElementIterator(Origin_Type &origin_type)
    -> ElementIterator<View_Type::origin, Origin_Type, 0>;

template <typename Origin_Type>
ElementIterator(Origin_Type &origin_type, int _index)
    -> ElementIterator<View_Type::runtime, Origin_Type, 0>;
...
public:
    ...
    Row_view(Origin_Type *_data, const size_t _cols)
        : data(_data), cols(_cols) {}
...
public:
    ...
    Row_view(Origin_Type *_data, const size_t _cols, const int _index)
        : data(_data), cols(_cols), index(_index) {}

    Row_view(Row<Origin_Type> r, const int _index)
        : data(r.data), cols(r.cols), index(_index) {}
...
// 推導指南
template <typename Origin_Type>
Row_view(Origin_Type *data, size_t cols)
    -> Row_view<View_Type::origin, Origin_Type, 0>;
...
public:
    RowIterator_view(Origin_Type *_data, const size_t _size)
        requires requires { TYPE != View_Type::runtime; }
        : data(_data), size(_size) {}

    RowIterator_view(Origin_Type *_data, const size_t _size, const int _index)
        requires requires { TYPE == View_Type::runtime; }
        : Optional_Index<TYPE>(_index), data(_data), size(_size) {}

    explicit RowIterator_view(RowIterator<Origin_Type> it)
        requires requires { TYPE != View_Type::runtime; }
        : data(it.data), size(it.size) {}

    RowIterator_view(RowIterator<Origin_Type> it, const int _index)
        requires requires { TYPE == View_Type::runtime; }
        : Optional_Index<TYPE>(_index), data(it.data), size(it.size) {}
    ...
    Row_view<TYPE, Origin_Type, Index> operator*() const {
        if constexpr (TYPE == View_Type::runtime) {
            return Row_view<TYPE, Origin_Type, Index>(
                data, size, Optional_Index<TYPE>::index);
        }
...
public:
    ...
    Matrix_view(Matrix<Origin_Type> &_origin, const int _index)
        : origin(&_origin), index(_index) {}
...
Matrix_view<View_Type::fixed, Origin_Type, Index> Matrix_view_fixed(
    Matrix<Origin_Type> &_origin) {
    return Matrix_view<View_Type::fixed, Origin_Type, Index>(_origin);
}

// 推導指引
template <typename Origin_Type>
Matrix_view(Matrix<Origin_Type> &_origin)
    -> Matrix_view<View_Type::origin, Origin_Type, 0>;

template <typename Origin_Type>
Matrix_view(Matrix<Origin_Type> &_origin, int _index)
    -> Matrix_view<View_Type::runtime, Origin_Type, 0>;
//                                              ^ 他不讓我加 const

};  // namespace f9ay

template <f9ay::View_Type TYPE, typename Origin_Type, int Index,
          typename Char_T>
struct std::formatter<f9ay::Matrix_view<TYPE, Origin_Type, Index>, Char_T>
    : std::formatter<std::string, Char_T> {
    auto format(const f9ay::Matrix_view<TYPE, Origin_Type, Index> &matrix,
                auto &ctx) const {
        auto out = ctx.out();
