...
template <typename T>
[[nodiscard]] T paethPredictor(T left, T above, T upperLeft) {
    // in theory the T will only one byte

    int p = static_cast<int>(left) + static_cast<int>(above) - static_cast<int>(upperLeft);
...
[[nodiscard]] inline Matrix<std::decay_t<decltype(std::declval<InputMatrix>()[0, 0])>> filter(
    ...
    for (int i = 0; i < matrix.row(); ++i) {
        ...
        for (int j = 0; j < matrix.col(); ++j) {
            ...
            T_val left = (j == 0) ? static_cast<T_val>(0) : matrix[i, j - 1];                     // byte to the left
            T_val above = (i == 0) ? static_cast<T_val>(0) : matrix[i - 1, j];                    // byte above
            T_val upperLeft = (i == 0 || j == 0) ? static_cast<T_val>(0) : matrix[i - 1, j - 1];  // byte upper-left
            ...
            switch (filterType) {
                ...
                case FilterType::Average: {
                    if constexpr (sizeof(T_val) == 4) {
                        ...
                        c -= (g + k) / 2;
                        d -= (h + l) / 2;
                        filteredByteVal = {static_cast<uint8_t>(a), static_cast<uint8_t>(b), static_cast<uint8_t>(c),
